...
| macOS 26.6.2（25G83） | Xcode 27.0（27A266a） | 通る¹ | **例外** | 通る |
| macOS 27.0（26A5406e） | Xcode 27.0（27A266a） | **例外** | **例外** | 通る |
| macOS 27.0 | Xcode 26.x | 未観測（その組み合わせを用意できない） | | |

1 行目と 3 行目はこのリポジトリの CI（`macos-26` と `xcode-27` ランナー）です。
この 2 つのイメージにはそれぞれ Xcode 26.6 と 27.0 しか入っておらず、OS と Xcode が
セットで変わるため、CI だけでは切り分けられません。2 行目は手元の Mac（Apple M4）
で、OS は 1 行目・ツールチェーンは 3 行目と同じ組み合わせです。

結論を決めているのは 2 行目です。OS を 26.6.2 に固定したまま Xcode だけ 26.6 →
27.0 に変えると、通っていたものが例外になります。つまり実行時の OS は変数では
ありません。4 行目はその裏返しですが、`xcode-27` ランナーには Xcode 26 が無く、
Apple もその組み合わせをサポートしていないため測れていません。1 行目と 2 行目で
結論は出ているので、無くても差し支えありません。
